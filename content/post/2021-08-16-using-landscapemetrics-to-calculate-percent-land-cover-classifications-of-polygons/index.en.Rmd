...
date: '2021-08-16'
slug: using-landscapemetrics-to-calculate-percent-land-cover-classifications-of-polygons
categories:
  - land use
  - landscape metrics
tags: []
...
##### To mitigate problems with the projections not lining up, use spTransform to match datasets. Afterwards, the next steps will calculate the percentage of each land cover in the raster. I pivot the table and reame the coloumns so it's easier to understand the output of this calculation.

```{r}
#match the spatial projectsion between RSPP and NLCD dataset 
...
##### Unfortunately, this shapefile had multiple small slivers in it. There are multiple roads surrounding this property and it is made up of multiple tax parcels, which caused this issue. Not a problem, we'll work with it as it is for now.
```{r}
print(map)
```
...
#join to unique IDS
#nlcd_rspp <- inner_join(rspp_cast, nlcd_wide, by= c("unique"="plot_id"))

shapefile(nlcd_wide, "nlcd_rspp.shp", overwrite = T)
st_write(nlcd_wide, "nlcd_rspp.shp", overwrite= T)
